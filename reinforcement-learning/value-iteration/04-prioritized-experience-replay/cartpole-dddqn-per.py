...
class DDDRQNAgent:
    def __init__(self, state_size, action_size):
        ...
        if self.load_model:
            self.model.load_weights("./save_model/cartpole_dddrqn_bi.h5")
...
if __name__ == "__main__":
    ...
    # Decide total number of states to use and number of states to burn away
    number_of_states = 4
    burn_away_states = 4
    ...
    burn_away_size = state_size * burn_away_states
    burned_in_state_size = burn_away_size + expanded_state_size 
    action_size = env.action_space.n

    agent = DDDRQNAgent(burned_in_state_size, action_size)
    ...
    for e in range(EPISODES):
        ...
        burned_in_state = np.zeros(burned_in_state_size)
        burned_in_next_state = np.zeros(burned_in_state_size)
        
        for h in range(state_size):
            expanded_state[(h + 1) * number_of_states -1] = state[h]
        
        # create burned-in state
        burned_in_state[0:burn_away_size] = expanded_state[0:burn_away_size]
        burned_in_state[burn_away_size:burned_in_state_size] = expanded_state
        
        # reshape states for LSTM input without embedding layer
        reshaped_state = np.zeros((1, burned_in_state_size, 2))
        for i in range(burned_in_state_size):
            for j in range(2):
                reshaped_state[0, i, j] = burned_in_state[i]
        ...
        while not done:
            ...
            # expand states for burn-in processing
            burned_in_next_state[0:burn_away_size] = expanded_next_state[0:burn_away_size]
            burned_in_next_state[burn_away_size:burned_in_state_size] = expanded_next_state
                        
            # reshape expanded next state for LSTM input without embedding layer
            reshaped_next_state = np.zeros((1, burned_in_state_size, 2))
            for i in range(burned_in_state_size):
                for j in range(2):
                    reshaped_next_state[0, i, j] = burned_in_next_state[i]
            ...
            if done:
                ...
                pylab.savefig("./save_graph/cartpole_dddrqn_bi.png")
        ...
        # save the model
        if e % 50 == 0:
            agent.model.save_weights("./save_model/cartpole_dddrqn_bi.h5")
